	...
	public List<History> getAll(History history, Paging paging) {
		...
		if (history != null) {
			if(history.getProductInfo() != null) {
				...
				if (history.getProductInfo().getCode() != null
						&& !StringUtils.isEmpty(history.getProductInfo().getCode())) {
					queryStr.append(" and model.productInfo.code=:code");
					mapParams.put("code", history.getProductInfo().getCode());
				}
				if (history.getProductInfo().getName() != null
						&& !StringUtils.isEmpty(history.getProductInfo().getName())) {
